...
public
  ...
  def self.zulutime(date,time)
    new_time=Time.local(date[0..3].to_i,
                        date[4..5].to_i,
                        date[6..7].to_i,
                        time[0..1].to_i,
                        time[2..3].to_i)
    date=new_time.gmtime.strftime("%Y%m%d")
    time=new_time.gmtime.strftime("%H%M")
    [date,time]
  end
  ...
  def self.query(channel,date,time)
    ...
    combined_hash=nil
    date,time=self.zulutime(date,time)
...
#["20090328", "20090404"].each do |j|
#  ["bbc7","radio4"].each do |k|
#    ["0000","0030","1000","1015","1800","1815","2200","2230","2300","2315"].each do |i|
#      puts k,j,i,TV_Anytime.whatson(k,j,i).inspect
#    end
#  end
#end

["Counterpoint", "Six O'Clock News"].each do |j|
  puts TV_Anytime.whenwas(j).inspect
  puts TV_Anytime.whenis(j).inspect
end
