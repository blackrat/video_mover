...
class IMDBWebPage
  ...
  def initialize(url=nil,year=nil,season=nil,episode=nil)
    FileUtils.mkdir_p(File.expand_path(IMDB_STORE)) unless File.exists?(File.expand_path(IMDB_STORE))
    @supplied_name=url
    @year=year
    @info=nil
    @page_type=:unknown
    if not url.nil?
      if url.is_url?
        ...
        @year||= begin url[/[^\d]\d{4}[^\d]*$/][/\d{4}/].to_i rescue nil end
        if @year
          url.gsub!(/\s*[^\d]\d{4}[^\d]*$/,'')
        end
...
private
  ...
  def fetch_yaml
    return nil if @supplied_name.nil? || @year
    @info||=(File.exists?("#{File.expand_path(File.join(IMDB_STORE,@supplied_name))}.yml") ? open("#{File.expand_path(File.join(IMDB_STORE,@supplied_name))}.yml") { |f| YAML.load(f) } : nil)
    return nil if @info.nil? || begin ((Time.now-@info[:last_updated]).to_i) > (60*60*24*7) rescue nil end
    @info
  end
...
#        open("#{File.expand_path(File.join(IMDB_STORE,@supplied_name))}.yml",'w') {|f| YAML.dump(@info,f)}
  ...
  def get_closest_match(doc)
    ...
    begin links[0][:link] rescue nil end
  end
    ...
    begin
      ...
      links=links.collect { |link|
        ...
        programme_type=case search_text
          when /TV mini-series/: :tv_mini_series
          when /TV series/: :tv_series
          when /TV/: :tv
          else :movie
        end
        position=position+1
  ...
  def expand_element(attribute,elements)
    begin
      case attribute
        when :creator,:creators: [:creator,expand_creator(elements)]
        when :director,:directors: [:director, expand_creator(elements)]
        when :writer,:writers: [:writer, expand_creator(elements)]
        when :seasons,:season: [:seasons,expand_all(elements)]
        when :parentsguide, :newsdesk, :genre, :plotkeywords, :awards, :usercomments, :country, :language, :color, :aspectratio, :certification, :filminglocations, :moviemeter, :company, :trivia, :goofs, :quotes, :releasedate, :alsoknownas, :soundmix, :movieconnections, :soundtrack: [attribute, expand_all(elements)]
        when :tagline, :plot, :runtime: [attribute, expand_text(elements)]
      else
