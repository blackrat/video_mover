...
class IMDB
  ...
  def expand_element(attribute,elements)
    begin
      case attribute
        when :creator,:creators: expand_creator(elements)
        when :director,:directors: expand_director(elements)
        when :writer,:writers: expand_writer(elements)
        when :seasons,:season: [:seasons,expand_all(elements)]
        when :genre: [:genre,expand_all(elements)]
        when :tagline: [:tagline,expand_text(elements)]
        when :plot: [:plot,expand_text(elements)]
        when :plotkeywords: expand_plot_keywords(elements)
        when :awards: expand_awards(elements)
        when :newsdesk: expand_newsdesk(elements)
        when :usercomments: expand_user_comments(elements)
        when :runtime: expand_runtime(elements)
        when :country: expand_runtime(elements)
        when :language: expand_language(elements)
        when :color: expand_runtime(elements)
        when :aspectratio: expand_aspect_ration(elements)
        when :certification: expand_certification(elements)
        when :filminglocations: expand_filming_locations(elements)
        when :moviemeter: expand_moviemeter(elements)
        when :company: expand_company(elements)
        when :trivia: expand_trivia(elements)
        when :quotes: expand_quotes(elements)
      else
