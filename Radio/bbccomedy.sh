...
#			bleb_get_details p_name p_desc $4 $2 1832
	...
	if [ -e ${g_mp3}/${name}/${long_name}.mp3 ] ; then
		...
		if [ -e ${g_wav}/${name}/${long_name}.wav ] ; then
			...
			make_dir ${g_wav}/${name}
			${DEBUG} mplayer -prefer-ipv4 -cache 40 -vc null -vo null -ao pcm:fast -ao pcm:file=${g_wav}/${name}/${long_name}.wav $1 
			if [ -e ${g_wav}/${name}/${long_name}.wav ] ; then
...
# Setup TV-Anytime programme listings for required dates
	...
	for g_time in 1130 1830 2300 2330
	do
	...
	for g_time in 1130 1200 1230 1400 1430 1800 1830 1900 1930 2000 2030 2200 2230 2300 2330
	do
...
set_init_value g_root ${ROOT_DIR} "/vault/hdc/radio/bbcradio"
set_init_value g_wav ${WAV_DIR} "/vault/hdc/radio/episodes"
set_init_value g_mp3 ${MP3_DIR} "/vault/hdc/radio/episodes"
set_init_value g_chk ${CHK_DIR} "/vault/hdc/radio/check"
set_init_value g_guide ${GUIDE_DIR} "/vault/hdc/radio/guide"
set_init_value g_radio4 ${RADIO4} "bbc_radio4"
set_init_value g_bbc7 ${BBC7} "bbc7"
set_init_value g_error_log ${ERROR_LOG} "/vault/hdc/radio/error.log"
set_init_value g_debug_log ${DEBUG_LOG} "/vault/hdc/radio/debug.log"
set_init_value g_lock_file ${LOCK} "/vault/hdc/radio/bbccomedy.lck"
